...
# TESTING DATA
tr = pd.read_csv("C:\\Users\\arpre\\Downloads\\Testing (1).csv")
tr.replace({'prognosis':{
    ...
    'Tuberculosis':25,
    'Common Cold':26,'Pneumonia':27,'Dimorphic hemmorhoids(piles)':28,'Heart attack':29,'Varicose veins':30,
    'Hypothyroidism':31,
...
X_test= tr[l1]
y_test = tr["prognosis"].values.flatten()
np.ravel(y_test)

# TRAINING DATA
df = pd.read_csv("C:\\Users\\arpre\\Downloads\\Training.csv\\Training.csv")
...
def NaiveBayes():
    ...
    print("Accuracy (non-normalized):", accuracy_score(y_test, y_pred, normalize=False))
